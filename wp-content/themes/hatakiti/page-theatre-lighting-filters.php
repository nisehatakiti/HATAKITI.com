...
(function(){
    ...
    function card(record){
        ...
        return '<article class="hk-gel-card"><a class="hk-gel-card-link" href="'+href+'">'+swatch(record)+'<div class="hk-gel-meta"><div class="hk-gel-brand">'+esc(record.manufacturer||'')+' / '+esc(record.series||'')+'</div><h3>'+esc(record.color_number||'')+' '+esc(record.color_name||'')+'</h3>'+(record.notes?'<p class="hk-gel-card-description">'+esc(record.notes)+'</p>':'')+'<dl><div><dt>透過率</dt><dd>'+esc(record.transmission_display||'未登録')+'</dd></div><div><dt>参考色</dt><dd>'+esc(record.reference_hex||'未登録')+'</dd></div></dl><div class="hk-gel-tags">'+uses+'</div><p class="hk-gel-card-note">※表示色はRGBによる参考色です。</p></div></a></article>';
    }
