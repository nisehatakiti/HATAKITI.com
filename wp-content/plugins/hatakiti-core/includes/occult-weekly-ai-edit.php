...
function hatakiti_build_occult_ai_prompt( $items, $week_start, $week_end ) {
    ...
    $system = <<<SYS
あなたは「週刊オカルト新聞」（HATAKITI.com）のAI編集者です。複数の情報源から集まった1週間分のオカルト関連ニュースを分析し、クラスタリング・重要度判定・記事執筆を行います。
...
【重要度判定】
各記事に、次の3段階のいずれかを付けてください。
- headline（大見出し）: 今週で最も重要・注目度の高い事件。複数ソースで報じられている、社会的注目度が高い、規模が大きい、続報性がある、特異性が高い等。
- major（主要記事）: 一定の関心を集める話題。
- minor（小記事）: 単独ニュースで扱いが小さいもの。
判断基準として、社会的注目度・事件の規模・情報源の数・続報性・オカルトジャンルとしての話題性・特異性・読者にとっての興味を考慮してください。

【記事執筆のルール — 著作権】
- 元記事の本文をそのまま長文コピー・引用してはいけません。
- 複数の情報を要約・比較・整理した、週刊オカルト新聞独自の編集記事を書いてください。単なる転載newsletterにしないでください。
- 本文は日本語で、300〜600字程度を目安にしてください。
- 「報じられている」「〜という」など、確認された事実と未確認情報を区別する表現を使ってください。事実を断定しすぎないでください。
...
function hatakiti_render_occult_ai_generate_page() {
    ...
    <div class="wrap hatakiti-record-form">
        <h1>AIで週刊号を作成</h1>
        <p class="description">
            対象期間内の、まだどの号にも使われていないニュースをAIに渡し、クラスタリング・重要度判定・記事執筆を行って、週刊号の<strong>下書き</strong>を作成します。
